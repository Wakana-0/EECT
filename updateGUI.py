...
def update_info(cv, window):
    ...
    update_text = maliang.Text(cv, (45, 0), text="EECT更新信息", fontsize=24)
    ...
    if cheek_version[0]:
        logger.info("检测到新版本")
        compare = "有可用更新！"
        download = maliang.Button(cv, (20, 260), text="下载更新", command=lambda: download_update(cv, window, cheek_version[6], cheek_version[7]))
    else:
...
def download_update(cv, window, name, url):
    ...
    cv.clear()
    wait = maliang.Spinner(cv, (20, 20), mode="indeterminate")
    update_text = maliang.Text(cv, (50, 20), text="正在下载更新，请稍等...")
    back = maliang.Button(cv, (20, 60), text="停止下载", command=lambda: stop(cv, window))
